...
use nativelink_proto::build::bazel::remote::execution::v2::execution_server::ExecutionServer as TonicExecutionServer;
use nativelink_service::execution_server::ExecutionServer;
use nativelink_util::task::TaskExecutor;
use nativelink_util::tls_utils::make_tcp_or_tls_stream;
use reverse_service::reverse_client::get_conn_after_upgrade;
use tonic::service::AxumRouter;
use tonic::transport::Server as TonicServer;

pub struct NewLocalWorker {
...
impl NewLocalWorker {
    pub fn new(
        config: LocalWorkerConfig,
        http: auto::Builder<TaskExecutor>,
        execution_service: TonicExecutionServer<ExecutionServer>,
    ) -> Result<Self, Error> {
        let svc = TonicServer::builder()
            .add_service(execution_service)
            .into_router();
        let uri = config.worker_api_endpoint.uri.parse::<Uri>().map_err(|e| {
            make_input_err!("Could not parse URI in NewLocalWorker: {e:?}")
        })?;
        Ok(Self {
            config,
            uri,
    ...
    pub async fn serve(&self) -> Result<(), Error> {
        let tls_config = &self.config.worker_api_endpoint.tls_config;
        // TODO! we should support the EndpointConfig.timeout field.
        let stream = make_tcp_or_tls_stream(&tls_config, &self.uri)
            .await
            .err_tip(|| "In NewLocalWorker")?;
        let request_builder = RequestBuilder::new()
            .method("GET")
            .uri(&self.uri)
            .version(Version::HTTP_11);
        let upgraded_stream = get_conn_after_upgrade(TokioIo::new(stream), request_builder)
            .await
            .map_err(|e| {
                make_input_err!("Could not upgrade connection in NewLocalWorker: {e:?}")
            })?;

        self.http
            .serve_connection(upgraded_stream, TowerToHyperService::new(self.svc.clone()))
            .await
            .map_err(|e| make_err!(Code::Internal, "Connection closed in NewLocalWorker: {e:?}"))
    }
}
