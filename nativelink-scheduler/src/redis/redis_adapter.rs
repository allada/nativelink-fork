...
use bytes::Bytes;
use fred::interfaces::{KeysInterface, PubsubInterface, SortedSetsInterface, TransactionInterface};
use fred::types::{RedisKey, RedisValue, SetOptions, ZRange, ZRangeBound, ZRangeKind, ZSort};
use futures::Stream;
...
pub enum RedisKeys<'a> {
    ...
    WorkerIdByOperationId(&'a OperationId),
}
...
impl<'a> std::fmt::Display for RedisKeys<'a> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::WorkerIdByOperationId(oid) => f.write_fmt(format_args!("wid:{oid}")),
        }
    }
}

impl<'a> From<RedisKeys<'a>> for RedisKey {
    fn from(val: RedisKeys<'a>) -> Self {
        Bytes::from(val.to_string()).into()
    }
}
...
impl RedisAdapter {
    pub fn new(store: Arc<RedisStore>, tasks_or_workers_change_notify: Arc<Notify>) -> Self {
        Self {
            operation_subscribers: RedisOperationSubscribers::new(
                store.clone(),
    ...
    pub async fn get_range_of_actions(
        ...
        ChunkedStream::new(start, end, move |start, end, mut output| async move {
            ...
            let sorted_actions_results: Vec<Result<SortedAwaitedAction, Error>> =
                result.iter().map(SortedAwaitedAction::try_from).collect();
            for sorted_action in sorted_actions_results.into_iter().flatten() {
                ...
                match self
                    ...
                    .get_operation_subscriber(&sorted_action.operation_id)
                    .await
                {
                    ...
                    _ => {
                        ...
                        let action_result =
                            self.get_bytes(&key).await.and_then(AwaitedAction::try_from);
                        if let Ok(action) = action_result {
                            let (tx, rx) = watch::channel(action);
                            self.operation_subscribers
                                .set_operation_sender(&sorted_action.operation_id, tx.clone())
                                .await;
                            output.push_back(RedisOperationSubscriber {
            ...
            Ok(Some(((new_start, new_end), output)))
        })
    }
    ...
    async fn add_new_operation(
        ...
        self.move_or_add_to_set(&sorted_state.to_string(), &sorted_action.to_string())
            .await
        ...
        let _: RedisValue = client
            .publish(pub_key, awaited_action_bytes)
            .await
            .err_tip(|| "In RedisAdapter::add_action")?;
        Ok(sub)
    }
    ...
    pub async fn update_awaited_action(
        ...
        let client = self.store.get_client();
        let worker_key = RedisKeys::WorkerIdByOperationId(new_awaited_action.operation_id());
        if new_awaited_action.state().stage == ActionStage::Queued {
            let tx = client.multi();
            let key: RedisKey =
                RedisKeys::WorkerIdByOperationId(new_awaited_action.operation_id()).into();
            tx.watch_before(key);
            let _: RedisValue = tx.del(worker_key.clone()).await?;
            let _: RedisValue = tx
                .exec(true)
                .await
                .err_tip(|| "In RedisAdapter::update_awaited_action - failed to remove worker")?;
        } else if let Some(new_worker) = new_awaited_action.worker_id() {
            let worker_bytes = Bytes::from(new_worker.to_string());
            let _: () = client
                .set(
                    worker_key.clone(),
                    worker_bytes,
                ...
                .await
                .err_tip(|| "In RedisAdapter::update_awaited_action - failed to set worker ID")?;
            let worker = self.get_bytes(&worker_key).await.err_tip(|| {
                "In RedisAdapter::update_awaited_action - failed to get worker ID after set"
            })?;

            if worker != new_worker.to_string() {
                return Err(make_err!(
                    Code::Internal,
                    "Expected worker to be set to {new_worker} but worker was {worker:?}"
                ));
            }
        }
        let tx = client.multi();
        let sorted_awaited_action = SortedAwaitedAction::from(&new_awaited_action);
        let watch_keys: Vec<RedisKey> = Vec::from([
            RedisKeys::AwaitedAction(new_awaited_action.operation_id()).into(),
            RedisKeys::WorkerIdByOperationId(new_awaited_action.operation_id()).into(),
            sorted_awaited_action.to_string().as_bytes().into(),
        ]);

        tx.watch_before(watch_keys);
        ...
        if new_awaited_action.state() != old_awaited_action.state() {
            let new_sorted_state =
                SortedAwaitedActionState::try_from(&new_awaited_action.state().stage)
                    .err_tip(|| "In RedisAdapter::update_awaited_action")?;
            let redis_key_sorted_aa: fred::types::RedisKey =
                sorted_awaited_action.to_string().as_bytes().into();
            let _: RedisValue = tx
                .zrem("Queued", redis_key_sorted_aa.clone())
                .await
                .err_tip(|| "In RedisAdapter::update_awaited_action - ZREM failed")?;
            let _: RedisValue = tx
                .zrem("Executing", redis_key_sorted_aa.clone())
                .await
                .err_tip(|| "In RedisAdapter::update_awaited_action - ZREM failed")?;
            let _: RedisValue = tx
                .zrem("Completed", redis_key_sorted_aa.clone())
                .await
                .err_tip(|| "In RedisAdapter::update_awaited_action - ZREM failed")?;
            let _: RedisValue = tx
                .zrem("CacheCheck", redis_key_sorted_aa.clone())
                .await
                .err_tip(|| "In RedisAdapter::update_awaited_action - ZREM failed")?;
            let _: RedisValue = tx
                .zadd(
        ...
        let _: RedisValue = tx
            .publish(pub_key, new_awaited_action_bytes.clone())
            .await
            .err_tip(|| "In RedisAdapter update_awaited_action")?;
